...
export default function DataPreviewTable({ rawText }: DataPreviewTableProps) {
            ...
            <table className="w-full text-xs">
                <thead className="sticky top-0 bg-slate-900 z-10">
                    <tr>
                        {displayColumns.map(col => (
                            <th key={col.name} className="text-left px-3 py-2 border-b border-slate-700 min-w-[120px]">
                                <div className="flex flex-col gap-0.5">
                                    <span className="font-bold text-slate-200 truncate">{col.name}</span>
                ...
                <tbody>
                    {paginatedRows.map((row, idx) => (
                        <tr
                            key={idx}
                            className="hover:bg-slate-800/50 cursor-pointer transition-colors border-b border-slate-800/50"
                        ...
                        >
                            {displayColumns.map(col => {
                                ...
                                return (
                                    <td key={col.name} className="px-3 py-2 text-slate-400 font-mono">
                ...
                <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between p-4 border-b border-slate-800">
                        <h3 className="text-sm font-bold text-white flex items-center gap-2">
                            <Table className="w-4 h-4 text-indigo-400" /> Row Details
                        </h3>
                        <button onClick={() => setSelectedRow(null)} className="text-slate-400 hover:text-white">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {messages && messages.length > 0 ? (
                            <div className="bg-slate-950/50 rounded-lg p-4 border border-cyan-800/30">
                            ...
                            Object.entries(selectedRow).map(([key, value]) => (
                                <div key={key} className="bg-slate-950/50 rounded-lg p-3 border border-slate-800">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="text-xs font-bold text-slate-300">{key}</span>
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded ${typeColors[detectType(value)]} bg-slate-800`}>
                                            {detectType(value)}
                                        </span>
                                    </div>
                                    <pre className="text-xs text-slate-400 font-mono whitespace-pre-wrap break-all max-h-48 overflow-y-auto">
    ...
    const ColumnPicker = () => {
        ...
        return (
            <div className="absolute right-0 top-8 z-20 bg-slate-900 border border-slate-700 rounded-lg shadow-xl p-3 min-w-[180px]">
                <div className="text-[10px] font-bold text-slate-400 uppercase mb-2">Visible Columns</div>
                <div className="space-y-1 max-h-48 overflow-y-auto">
                    {columns.map(col => (
                        <label key={col.name} className="flex items-center gap-2 cursor-pointer hover:bg-slate-800 p-1 rounded">
                            <input
                                type="checkbox"
        ...
        return (
            <>
                <div className="fixed inset-0 z-50 flex flex-col bg-slate-950">
                    ...
                    <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900">
                        <div className="flex items-center gap-4">
                            <h2 className="text-sm font-bold text-white flex items-center gap-2">
                                <Table className="w-4 h-4 text-indigo-400" /> Data Preview
                            </h2>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="pl-8 pr-3 py-1.5 text-xs bg-slate-800 border border-slate-700 rounded-lg text-slate-300 w-64 focus:border-indigo-500 outline-none"
                                />
                            </div>
                            <div className="relative">
                                <button
                    ...
                    {/* Pagination */}
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-4 p-4 border-t border-slate-800 bg-slate-900">
                            <button
                                onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                disabled={currentPage === 1}
                                className="p-2 rounded-lg hover:bg-slate-800 disabled:opacity-30 text-slate-400"
                            >
