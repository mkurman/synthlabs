    ...
    return (
        <>
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1 bg-slate-900/60 hover:bg-slate-800/70 border border-slate-700/70 text-slate-300 text-[9px] px-1.5 py-1 rounded transition-colors">
                        ...
                        <button onClick={onSaveRubric} className="flex items-center gap-1 bg-slate-900/60 hover:bg-slate-800/70 border border-slate-700/70 text-slate-300 text-[9px] px-1.5 py-1 rounded transition-colors">
                            <Save className="w-2.5 h-2.5" /> Save
                        </button>
                        <button onClick={onOptimizePrompt} disabled={isOptimizing} className="bg-sky-600/20 hover:bg-sky-600/40 text-sky-300 border border-sky-500/30 text-[9px] px-1.5 py-1 rounded flex items-center gap-1 transition-all">
                ...
                <textarea
                    ...
                    className="w-full h-40 bg-slate-950 border border-slate-700/70 rounded-lg p-2 text-[9px] font-mono text-slate-300 focus:border-sky-500 outline-none resize-y leading-relaxed"
                    spellCheck={false}
            ...
            {(dataSourceMode === DataSource.HuggingFace || dataSourceMode === DataSource.Manual) && (
                <div className="p-3 bg-slate-900/60 border border-slate-700/70 rounded-lg space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <input
                            ...
                            className="w-20 bg-slate-950 border border-slate-700/70 rounded px-2 py-1 text-xs text-slate-100 focus:border-sky-500 outline-none"
                            placeholder="All"
                        />
