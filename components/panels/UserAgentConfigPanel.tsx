        ...
        <div className="mt-4 pt-4 border-t border-slate-700/70">
            <div className="flex items-center justify-between mb-3">
                ...
                <button
                    ...
                    className={`w-10 h-5 rounded-full transition-all relative ${userAgentConfig.enabled ? 'bg-cyan-600' : 'bg-slate-800/70'}`}
                >
            ...
            {userAgentConfig.enabled && (
                <div className="space-y-3 p-3 bg-cyan-500/5 border border-cyan-500/20 rounded-lg animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] text-slate-400 font-bold uppercase">Responder</label>
                            <select
                                ...
                                className="w-full bg-slate-950 border border-slate-700/70 rounded px-2 py-1.5 text-xs text-white focus:border-cyan-500 outline-none"
                            >
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] text-slate-400 font-bold uppercase">User Agent Provider</label>
                        <div className="flex bg-slate-950 p-0.5 rounded border border-slate-700/70">
                            <button onClick={() => onUserAgentConfigChange(prev => ({ ...prev, provider: ProviderType.Gemini }))} className={`flex-1 py-1 text-[10px] font-medium rounded transition-all ${userAgentConfig.provider === ProviderType.Gemini ? 'bg-cyan-600 text-white' : 'text-slate-300'}`}>Gemini</button>
                            <button onClick={() => onUserAgentConfigChange(prev => ({ ...prev, provider: ProviderType.External }))} className={`flex-1 py-1 text-[10px] font-medium rounded transition-all ${userAgentConfig.provider === ProviderType.External ? 'bg-cyan-600 text-white' : 'text-slate-300'}`}>External</button>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                                ...
                                <select
                                    ...
                                    className="w-full bg-slate-950 border border-slate-700/70 rounded px-2 py-1 text-xs text-white outline-none"
                                >
