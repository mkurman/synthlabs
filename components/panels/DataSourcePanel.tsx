    ...
    return (
        <div className="space-y-4">
            <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800/70">
                <button onClick={() => onDataSourceModeChange(DataSource.Synthetic)} className={`flex-1 py-2 text-[10px] font-bold rounded flex flex-col items-center justify-center gap-1 transition-all uppercase tracking-wide ${dataSourceMode === DataSource.Synthetic ? 'bg-slate-900/80 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}><BrainCircuit className="w-3.5 h-3.5" /> Synthetic</button>
                <button onClick={() => onDataSourceModeChange(DataSource.HuggingFace)} className={`flex-1 py-2 text-[10px] font-bold rounded flex flex-col items-center justify-center gap-1 transition-all uppercase tracking-wide ${dataSourceMode === DataSource.HuggingFace ? 'bg-slate-900/80 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}><Server className="w-3.5 h-3.5" /> HuggingFace</button>
                <button onClick={() => onDataSourceModeChange(DataSource.Manual)} className={`flex-1 py-2 text-[10px] font-bold rounded flex flex-col items-center justify-center gap-1 transition-all uppercase tracking-wide ${dataSourceMode === DataSource.Manual ? 'bg-slate-900/80 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}><FileText className="w-3.5 h-3.5" /> Manual</button>
            </div>
