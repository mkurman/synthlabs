...
import { Download, Plus, Play, RefreshCw, Trash2 } from 'lucide-react';
...
export default function BatchPromptDebugger() {
    ...
    const handleExport = () => {
        const rowsWithResults = rows.map(row => ({
            ...
            result: results[row.id] || { status: 'idle' }
        }));
        ...
        link.href = url;
        link.download = `matrix-export-${Date.now()}.json`;
    ...
    return (
        <main className="h-full w-full overflow-y-auto p-4 pb-20 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
