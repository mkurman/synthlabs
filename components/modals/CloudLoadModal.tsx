...
export default function CloudLoadModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-slate-950/70 border border-slate-700/70 rounded-xl shadow-2xl max-w-2xl w-full p-6 max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Cloud className="w-5 h-5 text-sky-500" /> Cloud Sessions
          </h3>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-2">
            {sessions.map(session => (
              <div
                ...
                className="group bg-slate-950/70 border border-slate-800/70 hover:border-sky-500/50 p-3 rounded-lg cursor-pointer transition-all flex justify-between items-center"
              >
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] bg-slate-900/60 text-slate-300 px-1.5 py-0.5 rounded">
                      {session.config?.appMode === CreatorMode.Generator ? 'GEN' : 'CONV'}
                    </span>
                    {session.config?.engineMode === EngineMode.Deep && (
                      <span className="text-[10px] bg-sky-900/30 text-sky-400 px-1.5 py-0.5 rounded">
                        DEEP
                      </span>
                    )}
                  </div>
